...
# starmani olinuxino. LAN variant. kui sidet pole, teeb lpuks reboodi.

testserver="195.222.15.51"
langw1="62.65.245.97"  # kontroller ise 98
#langw2="10.0.0.254"
LOG=/root/d4c/chk_conn.log  # /mnt/mtd/chk_conn.log # will not be erased on reboot
...
if pingtest $testserver; then 
    echo conn ok; ip route; oklog $LOG
    #/bin/npe +USER_LED
    exit 0
else 
    echo conn still NOT ok
fi
...
if [ `echo "$lastlog" | tail -1 | grep "CONN_LOSS" | wc -l` -eq 0 ]; then # loss not detected before
    ...
    now=`date +%s`
    if [ `expr $now - $losstime` -gt 900 ]; then
